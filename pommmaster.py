...
# ==========================================
# FASTAPI ROUTES & ADMIN CONTROLLERS
# ==========================================
@app.get("/login", response_class=HTMLResponse)
async def login_form(request: Request, error: str | None = None):
    if is_authenticated(request):
        return RedirectResponse(url="/", status_code=status.HTTP_303_SEE_OTHER)
    return HTMLResponse(content=LOGIN_PAGE.replace("{err_html}", f'<div class="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-mono">{error}</div>' if error else ''))
...
async def admin_dashboard_view(request: Request, message: str | None = None):
    ...
    rendered_html = DASHBOARD_PAGE.replace("{status_pulse}", status_pulse)\
                                  ...
                                  .replace("{all_orders}", all_orders if 'all_orders' in locals() else "")\
