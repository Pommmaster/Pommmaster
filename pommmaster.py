...
SESSION_SECRET = os.getenv("SESSION_SECRET", "super_master_session_key_9988_xyz_secure")
...
# ==========================================
# TENANT DATABASE LAYER (V1 & V2 COMPATIBLE)
# ==========================================
...
async def init_tenant_db(slug: str, initial_token: str = ""):
    async with TENANT_DB_LOCKS[slug]:
        ...
        try:
            ...
            # Full 17 Multi-Packs setup for Pom Pom V2
            default_packs = [
...
# ==========================================
# BOT ENGINE (SUPPORTS BOTH V1 & V2 TEMPLATES)
# ==========================================
...
class MultiBotEngine:
    ...
    async def notify_payment_approved(self, slug: str, user_id: int, plan_name: str):
        ...
        try:
            ...
            logging.warning(f"[{slug}] Approval notification error: {e}")
        ...
        async def runner():
            while True:
                try:
                    ...
                    logging.error(f"[{slug}] Polling error: {e}")
...
# ==========================================
# MASTER LOGIN & DASHBOARD INTERFACES
# ==========================================
...
# ==========================================
# TENANT ROUTING FOR INDIVIDUAL PANELS
# ==========================================
...
async def tenant_login_page(slug: str, error: str | None = None):
    ...
    return HTMLResponse(LOGIN_PAGE.replace("Nagato Panel", client["client_name"]).replace('action="/login"', f'action="/c/{slug}/login"'))
...
async def tenant_admin_panel(
    ...
    try:
        ...
        context = {
            ...
            "plans": await get_tenant_all_plans(slug),
            "cfg": {"license_expiry": "2027-01-01 00:00:00"}
        }
        ...
        logging.error(f"[{slug}] Dashboard error: {err}")
...
async def tenant_save_general_settings(
    ...
    if cleaned_token: await update_tenant_setting(slug, "bot_token", cleaned_token)
    if welcome_text.strip(): await update_tenant_setting(slug, "welcome_text", welcome_text.strip())
    if plans_text.strip(): await update_tenant_setting(slug, "plans_text", plans_text.strip())
    if welcome_photo.strip(): await update_tenant_setting(slug, "welcome_photo", welcome_photo.strip())
    if demo_video.strip(): await update_tenant_setting(slug, "demo_video", demo_video.strip())
    ...
    return RedirectResponse(url=f"/c/{slug}/admin?message=Saved+successfully&tab=tab-media", status_code=status.HTTP_303_SEE_OTHER)
...
async def tenant_save_upi(slug: str, request: Request, upi_id: str = Form(...), payee_name: str = Form(...), admin_chat_id: str = Form("")):
    if not request.session.get(f"tenant_auth_{slug}"): return RedirectResponse(url=f"/c/{slug}/login", status_code=status.HTTP_303_SEE_OTHER)
    ...
    return RedirectResponse(url=f"/c/{slug}/admin?message=UPI+saved&tab=tab-settings", status_code=status.HTTP_303_SEE_OTHER)

@app.post("/c/{slug}/admin/plans/add")
async def tenant_add_plan_endpoint(slug: str, request: Request, plan_id: str = Form(...), name: str = Form(...), amount: float = Form(...), validity: str = Form(...), access_link: str = Form("")):
    if not request.session.get(f"tenant_auth_{slug}"): return RedirectResponse(url=f"/c/{slug}/login", status_code=status.HTTP_303_SEE_OTHER)
