...
async def handle_admin_upload_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text(f" Usage: `/upload <number 1-{total_btns}>`\nExample: `/upload 1`", parse_mode="Markdown")
        return

    btn_num = int(context.args[0])
    if not (1 <= btn_num <= total_btns):
        await update.message.reply_text(f" Button number must be between 1 and {total_btns}.")
        return
...
async def handle_admin_clear_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total_btns = len(cfg["buttons"])
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text(f" Usage: `/clear <1-{total_btns}>`")
        return
    ...
    if 0 <= btn_idx < total_btns:
        ...
        await update.message.reply_text(f" Cleared all media for Button #{btn_idx + 1}.")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "btn_home":
        ...
        if fallback_urls:
            watch_keyboard = [
                [InlineKeyboardButton(f" Watch Preview Clip {i}", url=link)]
        ...
        if not session:
            await context.bot.send_message(
                chat_id=chat_id,
                text=" Session expired. Please choose a package again.",
                reply_markup=InlineKeyboardMarkup([[
...
async def admin_dashboard_view(request: Request, message: str | None = None):
    ...
    for idx in range(num_buttons):
        ...
        # Standalone clean form cards with no nested forms
        pack_cards_html += f"""<div class="glass-card rounded-2xl p-4 flex flex-col justify-between space-y-3 border border-purple-900/40 relative group hover:border-cyan-400/50 transition">
            <div class="flex items-center justify-between border-b border-purple-900/30 pb-2">
                <span class="text-xs font-tech text-cyan-400 font-bold tracking-wider"> BUTTON #{idx + 1}</span>
                <span class="text-[10px] font-mono text-purple-300 bg-purple-950/60 px-2 py-0.5 rounded border border-purple-800/40">{len(v_list)} media items</span>
            </div>
            
            <div class="space-y-2 text-xs">
                <div>
                    <label class="block text-[10px] font-mono text-purple-300 uppercase mb-1">Button Label (In Bot)</label>
                ...
                <div>
                    <label class="block text-[10px] font-mono text-purple-300 uppercase mb-1">Description (Preview Card)</label>
                ...
                <div>
                    <label class="block text-[10px] font-mono text-cyan-300 uppercase font-semibold mb-1"> Access Link (Delivered Upon Payment)</label>
                    <input type="url" name="pack_link_{idx}" value="{delivery_link}" placeholder="https://t.me/+..." class="w-full bg-[#070410] border border-cyan-500/40 rounded-lg px-2.5 py-1.5 text-cyan-300 text-xs focus:outline-none focus:border-cyan-400">
                </div>
                <div>
                    <label class="block text-[10px] font-mono text-purple-300 uppercase mb-1">Stored Media (File ID or URL, 1 per line)</label>
                    <textarea name="btn_videos_{idx}" rows="2" placeholder="file_id or http://..." class="w-full bg-[#070410] border border-purple-900/60 rounded-lg px-2.5 py-1.5 text-white text-[10px] font-mono focus:outline-none focus:border-cyan-400">{v_text}</textarea>
                </div>
                <div class="pt-2 flex justify-end">
                    <button type="button" onclick="deleteSingleButton({idx})" class="text-[10px] font-mono text-rose-400 hover:text-rose-200 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/40 px-3 py-1 rounded-lg transition flex items-center gap-1">
                         Delete Button #{idx + 1}
                    </button>
                </div>
            </div>
...
<body class="text-slate-100 min-h-screen flex overflow-x-hidden relative">
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto relative z-10">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <div id="tab-media" class="tab-content space-y-5 hidden">
                <form method="POST" action="/admin/save-message-settings" class="glass-card rounded-2xl p-6 space-y-5">
                    ...
                    <div class="border-t border-purple-900/40 pt-4 space-y-4">
                        <h4 class="font-tech text-xs font-bold text-cyan-400 tracking-wider">Bottom Action Buttons</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div id="tab-plans" class="tab-content space-y-5 hidden">
                <div class="glass-card rounded-2xl p-5 border border-cyan-500/40">
                    ...
                    <form method="POST" action="/admin/plans/add-button" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
                        <div>
                            ...
                            <input type="text" name="new_btn_label" required placeholder="e.g. ULTRA VIP" class="w-full bg-[#070410] border border-purple-900/60 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-cyan-400">
                        </div>
                        <div>
                            <label class="block text-[10px] font-mono text-purple-300 uppercase">Pack Name</label>
                            <input type="text" name="new_pack_name" required placeholder="e.g. Ultra VIP Pack" class="w-full bg-[#070410] border border-purple-900/60 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-cyan-400">
                        </div>
                        <div>
                            <label class="block text-[10px] font-mono text-purple-300 uppercase">Price ()</label>
                            <input type="text" name="new_pack_price" required placeholder="e.g. 499" class="w-full bg-[#070410] border border-purple-900/60 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-cyan-400">
                        </div>
                ...
                <!-- Main Save Form (File inputs removed from bulk submission to guarantee reliable saves) -->
                <form id="savePacksForm" method="POST" action="/admin/save-packs" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {pack_cards_html}
                    </div>
                    
                    <div class="sticky bottom-4 z-20 pt-2">
                        <button type="submit" class="w-full bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 hover:from-emerald-400 hover:to-cyan-400 text-white font-tech font-bold py-4 rounded-2xl shadow-[0_0_25px_rgba(16,185,129,0.35)] tracking-wider uppercase transition text-sm">
                             Save All Plan Buttons
                        </button>
                    </div>
                </form>

                <!-- Dedicated Single Deletion Form -->
                <form id="deleteButtonForm" method="POST" action="/admin/plans/delete-button" style="display:none;">
    ...
    <script>
        ...
        function deleteSingleButton(idx) {{
            if (confirm("Are you sure you want to delete Button #" + (idx + 1) + "?")) {{
                document.getElementById('deleteBtnIndexInput').value = idx;
                document.getElementById('deleteButtonForm').submit();
            }}
        }}
