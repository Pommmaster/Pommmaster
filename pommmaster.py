...
def parse_date_flexibly(date_str: str) -> datetime.date:
    """Parses date strings whether formatted as YYYY-MM-DD, DD/MM/YYYY, or DD-MM-YYYY."""
    if not date_str:
...
async def is_tenant_subscription_valid(slug: str) -> tuple[bool, str, str]:
    """Strict expiry check: halts bots and deactivates panels when days_left <= 0."""
...
<body class="min-h-screen flex items-center justify-center p-4">
    <div class="max-w-md w-full bg-[#0d071d] border border-rose-500/50 rounded-2xl p-8 text-center space-y-4 shadow-[0_0_35px_rgba(244,63,94,0.3)]">
        ...
        <p class="text-xs text-purple-300 font-mono">
            Contact the reseller or developer to renew access.
        </p>
        <div class="pt-2">
...
<body class="text-slate-100 min-h-screen flex">
    ...
    <script>
        ...
        function openManageModal(id, name, slug, telegram, expiry, clientStatus, adminUser, adminPass, token, botType) {
            ...
            // Normalize any DD/MM/YYYY dates to YYYY-MM-DD for standard date input
            var expVal = expiry || '';
            if (expVal.indexOf('/') !== -1) {
    ...
    async with MASTER_DB_LOCK:
        ...
        try:
            ...
            await db.execute("""
                ...
                WHERE id = ?
            """, (client_name.strip(), telegram.strip(), client_status, clean_expiry_str, admin_username.strip(), admin_password.strip(), cleaned_token, bot_type, client_id))
            await db.commit()
        finally:
            await db.close()
