    ...
    if data == "btn_home":
        ...
        if not session:
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(DEFAULT_BACK_BUTTON, callback_data="btn_home")
                ]])
            )
            return
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <div id="tab-dashboard" class="tab-content active space-y-6">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    ...
                    <div class="glass-card rounded-2xl p-4 flex flex-col justify-between neon-border-pink">
                        <span class="font-tech text-lg md:text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-cyan-300">ACTIVE</span>
            ...
            <div id="tab-broadcast" class="tab-content space-y-6">
                <form method="POST" action="/admin/broadcast/send" class="glass-card rounded-2xl p-6 space-y-5">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-semibold text-purple-300 uppercase mb-2 font-mono">Broadcast Text *</label>
                            <textarea name="broadcast_message" rows="4" required placeholder="Type announcement message..." class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl p-3 text-sm text-white"></textarea>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-semibold text-purple-300 uppercase mb-2 font-mono">Photo URL or file_id</label>
                                <input type="text" name="broadcast_photo" placeholder="https://..." class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl p-2.5 text-sm text-white">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-purple-300 uppercase mb-2 font-mono">Video URL or file_id</label>
                                <input type="text" name="broadcast_video" placeholder="https://..." class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl p-2.5 text-sm text-white">
                            </div>
                        </div>
                    </div>
                    <button type="submit" onclick="return confirm('Send mass broadcast to all users?');" class="bg-gradient-to-r from-fuchsia-600 to-purple-600 text-white font-tech font-bold px-6 py-2.5 rounded-xl text-xs uppercase tracking-wider">
                        📢 Transmit Broadcast
                    </button>
                </form>
            </div>
                ...
                <form method="POST" action="/admin/save-bot-settings" class="glass-card rounded-2xl p-6 space-y-5">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="upi_id" value="{cfg['upi_id']}" required class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl p-3 text-sm text-white font-mono">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-purple-300 mb-2 font-mono uppercase">Payee Name</label>
                            <input type="text" name="payee_name" value="{cfg['payee_name']}" required class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl p-3 text-sm text-white">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-purple-300 mb-2 font-mono uppercase">Admin Chat ID</label>
                            <input type="text" name="admin_chat_id" value="{cfg['admin_chat_id']}" class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl p-2.5 text-sm text-white font-mono">
                        </div>
                    </div>
                    <button type="submit" class="bg-gradient-to-r from-fuchsia-600 to-purple-600 text-white font-tech font-bold px-5 py-2.5 rounded-xl text-xs uppercase">Save Settings</button>
                </form>
            </div>
            ...
            <div id="tab-bot" class="tab-content space-y-5">
                <form method="POST" action="/admin/save-bot-settings" class="glass-card rounded-2xl p-6 space-y-5">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-purple-300 uppercase mb-2 font-mono">Bot Token</label>
                        <input type="text" name="token" value="{cfg['token']}" required class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl p-3 text-sm text-white font-mono">
                    </div>
                    <button type="submit" class="bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-tech font-bold px-5 py-2.5 rounded-xl text-xs uppercase">Update &amp; Restart Bot</button>
                </form>
            </div>
            ...
            <div id="tab-media" class="tab-content space-y-5">
                <form method="POST" action="/admin/save-message-settings" class="glass-card rounded-2xl p-6 space-y-5">
                    <h3 class="font-tech text-base font-bold text-white tracking-wide border-b border-purple-900/40 pb-3">Interface Templates</h3>
                    <div>
                        <label class="block text-xs font-semibold text-purple-300 uppercase mb-2 font-mono">Welcome Caption</label>
                        <textarea name="welcome_caption" rows="4" required class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl p-3 text-sm text-white">{cfg['welcome_caption']}</textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-purple-300 uppercase mb-2 font-mono">Banner Image URLs</label>
                        <textarea name="welcome_images" rows="3" class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl p-3 text-sm text-white">{welcome_images_text}</textarea>
                    </div>
                    <button type="submit" class="w-full bg-gradient-to-r from-fuchsia-600 to-cyan-600 text-white font-tech font-bold py-3 rounded-xl uppercase">Save Templates</button>
                </form>
            </div>

            <!-- TAB: PLANS -->
            <div id="tab-plans" class="tab-content space-y-5">
                <div class="glass-card rounded-2xl p-5 border border-cyan-500/40 space-y-3">
                    <h3 class="font-tech text-sm text-cyan-300 font-bold">➕ Add New Plan Button</h3>
                    <form method="POST" action="/admin/plans/add-button" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
                        <input type="text" name="new_btn_label" required placeholder="Button Label" class="bg-[#070410] border border-purple-900 rounded p-2 text-xs text-white">
                        <input type="text" name="new_pack_name" required placeholder="Pack Name" class="bg-[#070410] border border-purple-900 rounded p-2 text-xs text-white">
                        <input type="text" name="new_pack_price" required placeholder="Price (₹)" class="bg-[#070410] border border-purple-900 rounded p-2 text-xs text-white">
                        <button type="submit" class="bg-cyan-600 font-tech font-bold py-2 rounded text-xs uppercase text-white">Add Button</button>
                    </form>
                </div>
                <form method="POST" action="/admin/save-packs" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <button type="submit" class="w-full bg-emerald-600 text-white font-tech font-bold py-3.5 rounded-xl uppercase">Save All Buttons &amp; Media</button>
                </form>
            </div>
        </div>
    </main>
...
async def handle_user_subscription_change(request: Request, chat_id: int = Form(...), plan_name: str = Form(...)):
    if not is_authenticated(request):
        return RedirectResponse(url="/login", status_code=status.HTTP_303_SEE_OTHER)
    update_user_subscription(chat_id, plan_name)
    return RedirectResponse(url=f"/?tab=tab-users&message=Subscription+updated", status_code=status.HTTP_303_SEE_OTHER)

@app.post("/admin/users/ban")
async def handle_user_ban_toggle(request: Request, chat_id: int = Form(...), status_val: int = Form(..., alias="status")):
    if not is_authenticated(request):
        return RedirectResponse(url="/login", status_code=status.HTTP_303_SEE_OTHER)
    set_user_ban(chat_id, status_val)
    return RedirectResponse(url=f"/?tab=tab-users&message=User+status+updated", status_code=status.HTTP_303_SEE_OTHER)

@app.post("/admin/plans/add-button")
async def handle_add_plan_button(request: Request, new_btn_label: str = Form(...), new_pack_name: str = Form(...), new_pack_price: str = Form(...)):
    if not is_authenticated(request):
        return RedirectResponse(url="/login", status_code=status.HTTP_303_SEE_OTHER)
    cfg = get_settings()
    buttons = cfg["buttons"]
    ...
    button_details[new_idx] = {"pack": new_pack_name.strip(), "price": new_pack_price.strip(), "desc": "Full VIP pack access.", "link": ""}
    ...
    return RedirectResponse(url="/?tab=tab-plans&message=New+button+added", status_code=status.HTTP_303_SEE_OTHER)

@app.post("/admin/plans/delete-button")
async def handle_delete_plan_button(request: Request, btn_index: int = Form(...)):
    if not is_authenticated(request):
        return RedirectResponse(url="/login", status_code=status.HTTP_303_SEE_OTHER)
    cfg = get_settings()
    buttons = cfg["buttons"]
    ...
    if 0 <= btn_index < len(buttons):
        ...
        final_videos, final_details = {}, {}
        curr_new = 0
        for old_i in range(len(buttons)):
            if old_i == btn_index: continue
            if str(old_i) in button_videos: final_videos[str(curr_new)] = button_videos[str(old_i)]
            if str(old_i) in button_details: final_details[str(curr_new)] = button_details[str(old_i)]
            curr_new += 1
    ...
    return RedirectResponse(url="/?tab=tab-plans&message=Button+deleted", status_code=status.HTTP_303_SEE_OTHER)

@app.post("/admin/broadcast/send")
async def handle_admin_broadcast(request: Request, broadcast_message: str = Form(...), broadcast_photo: str = Form(""), broadcast_video: str = Form("")):
    if not is_authenticated(request):
        return RedirectResponse(url="/login", status_code=status.HTTP_303_SEE_OTHER)
    if not bot_manager.app:
        return RedirectResponse(url="/?tab=tab-broadcast&message=Error:+Bot+offline", status_code=status.HTTP_303_SEE_OTHER)
    ...
    for (uid,) in users:
        try:
            if broadcast_video.strip():
                await bot_manager.app.bot.send_video(chat_id=uid, video=broadcast_video.strip(), caption=broadcast_message, supports_streaming=True)
            elif broadcast_photo.strip():
                await bot_manager.app.bot.send_photo(chat_id=uid, photo=broadcast_photo.strip(), caption=broadcast_message)
            else:
...
async def save_bot_settings(request: Request, token: str = Form(...), upi_id: str = Form(...), payee_name: str = Form(...), admin_chat_id: str = Form("")):
    ...
    return RedirectResponse(url="/?tab=tab-settings&message=Settings+saved", status_code=status.HTTP_303_SEE_OTHER)

@app.post("/admin/save-message-settings")
async def save_message_settings(request: Request, welcome_caption: str = Form(...), welcome_images: str = Form(""), btn_how_to_use: str = Form(...), msg_how_to_use: str = Form(...), btn_report_issue: str = Form(...), msg_report_issue: str = Form(...), btn_language: str = Form(...), msg_language: str = Form(...)):
    if not is_authenticated(request):
        return RedirectResponse(url="/login", status_code=status.HTTP_303_SEE_OTHER)
    ...
    return RedirectResponse(url="/?tab=tab-media&message=Templates+saved", status_code=status.HTTP_303_SEE_OTHER)
...
async def save_packs_settings(request: Request):
    ...
    update_field("button_videos_json", json.dumps(button_videos))
    update_field("button_details_json", json.dumps(button_details))
    return RedirectResponse(url="/?tab=tab-plans&message=Plans+saved+successfully", status_code=status.HTTP_303_SEE_OTHER)
...
async def reset_revenue(request: Request):
    ...
    conn.commit()
    conn.close()
    return RedirectResponse(url="/?tab=tab-dashboard&message=Revenue+reset", status_code=status.HTTP_303_SEE_OTHER)
