...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "btn_home":
    ...
    elif data.startswith("v2_cat_") or data.startswith("btn_cat_"):
        await query.answer()
        btn_idx = data.replace("v2_cat_", "").replace("btn_cat_", "")
        ...
        payment_markup = InlineKeyboardMarkup([
            ...
            [
                InlineKeyboardButton(
                    DEFAULT_BACK_BUTTON,
                    callback_data=f"v2_cat_{btn_idx}",
                    api_kwargs={"style": "primary"}
                )
            ]
        ])
        ...
        if not session:
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(DEFAULT_BACK_BUTTON, callback_data="v2_home", api_kwargs={"style": "primary"})
                ]])
            )
            return
